...
%To further demonstrate how a runtime works, we first shows the internal structure of a runtime, followed by its overall work flow to process data-plane flows and remote actor messages. Finally, we discuss the service chain model adopted by each runtime.
...
The coordinator in \nfactor~is responsible for basic cluster management routines. %, e.g., monitoring system workload, updating cluster composition,  dynamically scaling runtimes for deployed clusters, and recovering failed runtime in a cluster.
