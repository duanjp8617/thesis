...
def draw():
	...
	netstar=[6629596,6271063,6345558,2551231]
	netstar = map(lambda f:f/1000000.0, netstar)

#	opennf=map(float,opennf)
	callback=[6756884,6367254,6414090,2552543]
...
#	nfa=map(float,nfa)
	...
	plt.xlabel("Input packet size: 128 Byte", fontsize=25, style='normal', color='black')
	plt.ylabel("Throughtput(Mpkts/s)", fontsize=25, style='normal', color='black')
	plt.savefig("StatelessNF_throughput_comparison_128byte.pdf", bbox_inches='tight', pad_inches=0)
	plt.show()
