...
\noindent I declare that this thesis represents my own work, except where due acknowledgement is made, and that it has not been previously included in a thesis, dissertation or report submitted to this University or to any other institution for a degree, diploma or other qualiﬁcations.
...
Despite of the advantages, there has been several problems that bother the practitioners of NFV technology. First, some network services are preferred to be deployed across geo-distributed datacenters so that users can enjoy better service quality. However, existing NFV management systems suffer poor scalability when deploying NFV service chains across geo-distributed datacenters, as scaling across multiple datacenters requires non-trivial interactions between various components of an NFV management system. Second, resilience functionality, including failure resilience and flow migration, is of pivotal importance in NFV system. But existing NFV systems implementing resilience functionalities usually have unsatisfactory performance due to heavyweight check-pointing and centralized control. Finally, some NFs need to contact external services to achieve advanced functionalities. For the best performance, these NFs are usually implemented using callback-based asynchronous programming method, which is complex and error-prone. Moreover, the increased implementation complexity brought by callbacks further limits the development of better NF software.
...
\input{chap-background/background.tex}
